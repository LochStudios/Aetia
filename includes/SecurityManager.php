...
class SecurityManager {
    ...
    public function validateBillingData($billData) {
        ...
        foreach ($billData as $client) {
            ...
            foreach ($requiredFields as $field) {
                ...
                // String fields must be non-empty after trim
                if (in_array($field, ['email', 'username'], true) && trim((string)$client[$field]) === '') {
                    throw new SecurityException("Missing required field: $field");
                }
            }
    ...
    private function getSecretKey() {
        ...
        $envSecret = getenv('AETIA_TOKEN_SIGNING_KEY');
        if (is_string($envSecret) && $envSecret !== '') {
            $secretKey = $envSecret;
        }
        // Use configured secret key when available
        $configFile = '/home/aetiacom/web-config/security.php';
        if (file_exists($configFile)) {
            include $configFile;
            if (isset($tokenSigningKey) && is_string($tokenSigningKey) && $tokenSigningKey !== '') {
                $secretKey = $tokenSigningKey;
            }
        }
        // Deterministic fallback to avoid undefined-variable/runtime failures
        if ($secretKey === null) {
            $secretKey = hash('sha256', __FILE__ . '|' . php_uname() . '|' . PHP_VERSION);
        }
        return $secretKey;
    }
}
